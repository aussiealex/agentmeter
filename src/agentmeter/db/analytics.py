...
def get_project_stats(
    ...
    """Get aggregated stats per project."""
    clauses: list[str] = []
    params: list[str] = []
    ...
    # Exclude empty project — those are untagged calls
    clauses.append("project != ''")
    ...
    return [
        ProjectStats(
            ...
            error_count=r["error_count"] or 0,
            total_elapsed_ms=r["total_elapsed_ms"] or 0,
            first_call=r["first_call"] or "",
            last_call=r["last_call"] or "",
        )
        for r in rows
    ]
...
def get_project_tool_breakdown(
    ...
    """Get per-tool stats for a specific project."""
    clauses = ["project = ?"]
    params: list = [project]
